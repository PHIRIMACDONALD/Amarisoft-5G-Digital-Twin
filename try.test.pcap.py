    ...
    try:
        for file_pattern in file_patterns:
            ...
            if container_target:
                ...
                try:
                    subprocess.run(docker_rm_command, shell=True, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def transfer_files_concurrently(counter):
    with ThreadPoolExecutor() as executor:
        ...
        for file, container_path in current_files.items():
            ...
            futures.append(executor.submit(transfer_file, remote_file_path, local_file_path, file))
